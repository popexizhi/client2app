...
class sh_dev():
    ...
    def dev_provision_check(self, start_num, num, res):
        """
    ...
    def dev_start(self):
        """
                start dev ./slim_engine_test -cfg="ue_%d.cfg" ="%d" % (self.host_id, self.host_id)
        """
        dev_sta_sh = './slim_engine_test -cfg="ue_%d.cfg" -host=%d ' % (self.host_id, self.host_id)
        L2_STA = 'CreateSocketStream host is'
        print dev_sta_sh
        self.pexpect = pexpect.spawn(dev_sta_sh)
        self.pexpect.expect(L2_STA)
    ...
    def dev_send_data(self, send_num):
        """
        test client(1809 )send count ---\d+ .* 4 total_send_size=4000
        """
        assert self.pexpect
        SEND_PACKAGES = 'send count ---\d+ .* %d total_send_size=' % (int(send_num)+1)
        print "** " * 20
        print SEND_PACKAGES 
        self.pexpect.expect(SEND_PACKAGES)
        print self.pexpect.after
